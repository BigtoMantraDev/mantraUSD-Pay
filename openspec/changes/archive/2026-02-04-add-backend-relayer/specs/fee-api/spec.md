...
The Fee API provides fee quotes for gasless transaction relay. Fees cover the gas costs paid by the relayer and include a configurable buffer for gas price volatility.
...
### Performance
- Fee quote generation SHOULD complete in < 500ms
- Gas price lookups SHOULD be cached for 10 seconds

### Accuracy
- Fee calculations SHALL use at least 18 decimal precision
- Final fee SHALL be rounded up to token decimals (6 for mantraUSD)

### Availability
- Fee endpoint SHOULD have 99.9% uptime
- Fallback gas price SHALL be used if oracle fails
